...
{ // begin namespace cg
  ...
  // =========
  RayTracer::RayTracer(SceneBase &scene, Camera &camera) : Renderer{scene, camera},
  ...
  //[]---------------------------------------------------[]
  {
    ...
    if (m->transparency != Color::black)
    {
     ...
     // color += m->specular * trace(reflectionRay, level + 1, weight);
      ...
      auto n12 = n1 / n2;
      auto C2 = sqrt(1 - ((n12 * n12) * (1 - (C1 * C1)))); // Equation (2)

      auto T = ((n12 * L)) + ((n12 * C1 - C2) * N); // Equation (1) origem
      if (weight > _minWeight && level < _maxRecursionLevel)
      {

        auto refractionRay = Ray3f{P + T.normalize() * rt_eps(), T.normalize()};

        //auto Ot = trace(refractionRay, level + 1, weight); 
      ...
      //[]---------------------------------------------------[]
  {
      Intersection hit;
      auto ray2 = ray;
      while (_bvh->intersect(ray2, hit))
      {
          auto P = ray2(hit.distance + rt_eps());
          auto primitive = (Primitive*)hit.object;
         ...
         if (m->transparency == Color::black)
          ...
          auto N = primitive->normal(hit);
          auto dotp = N.dot(ray2.direction);
         

          if (dotp >= -0.001 && dotp <= 0.001)
          {
                  ++_numberOfHits;
                  return false;
          }
